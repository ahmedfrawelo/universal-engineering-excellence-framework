...
- Browser work means the user's existing Chrome only—never a second browser, profile, context, Cursor/IDE Simple Browser, in-app browser, or connector-created window. For Chrome tasks, prefer the installed Chrome control plugin against the user's existing tabs/profile (Codex Chrome plugin, or Claude `mcp__node_repl__js` bridge when present), then use the extension binding, enumerate `user.openTabs()`, and `claimTab()` the verified user-owned tab. Explicitly forbid Playwright MCP, chrome-devtools MCP, `browser_*`/Cursor/IDE Simple Browser tools, `browser.newContext`, and `browser.launch` as substitutes; Playwright is allowed only through the claimed tab's `tab.playwright` API when the Claude bridge is in use.
