...
- Before `COMPLETE` or 100 percent, validate a completion-audit artifact with `scripts/validate-completion-audit.ps1`; it must map every explicit requirement to passing acceptance evidence and contain no remaining work or known problems. Build/test success alone is insufficient.
...
Browser hard stop:
- HARD FAIL BEFORE ANY BROWSER TOOL: only when the task explicitly needs the browser. For a task tagged `browser`, run `scripts/get-ueef-task-preflight.ps1 -Task <task> -TaskTag browser` and resolve its `browserGate` checklist before selecting or calling any browser tool. The user-owned Chrome path must use the platform's installed Chrome control plugin against the user's existing tabs/profile: on Codex, the Chrome control plugin; on Claude, the `mcp__node_repl__js` -> installed Chrome extension binding -> exact `user.openTabs()` object -> `claimTab()` -> claimed `tab.playwright` bridge. If the gate is absent, unresolved, or Chrome control is unavailable, stop and ask; do not select a browser tool, do not invent a fallback browser, and do not open an alternate surface.
...
- Browser work means the user's existing Chrome only—never a second browser, profile, context, Cursor/IDE Simple Browser, in-app browser, or connector-created window. For Chrome tasks, use the installed Chrome control plugin, extension binding, exact `user.openTabs()` object, and `claimTab()` first. Playwright MCP, `browser_*`, `browser.newContext`, and `browser.launch` remain forbidden substitutes. A directly exposed Chrome DevTools/CDP adapter is allowed only as `AUTHORIZED_LOOPBACK_LAST_RESORT`: every same-tab channel in `config/browser-emergency-fallback.json` has recorded failure evidence, the user explicitly authorized the fallback, and `scripts/get-remote-debugging-readiness.ps1 -AuthorizedLastResort -PriorStageFailure <recorded-stages>` returned `READY_LAST_RESORT`. Endpoint and WebSocket must be loopback-only, the same existing target is required, and browser launch or cookie/storage/profile inspection remains forbidden. Host or installed-skill prohibitions still win.
