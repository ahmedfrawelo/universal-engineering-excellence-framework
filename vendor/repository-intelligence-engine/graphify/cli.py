    ...
    try:
        if kind == "search":
            ...
            if explicit:
                ...
                for v in explicit:
                    p = Path(v)
                    # Persisted/tool paths can use POSIX separators on Windows;
                    # a slash-rooted path is still absolute and must not be
                    # treated as an in-project relative read.
                    if not p.is_absolute() and not v.replace("\\", "/").startswith("/"):
