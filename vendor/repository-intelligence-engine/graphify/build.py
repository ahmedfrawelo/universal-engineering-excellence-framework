...
def _semantic_id_remap(nodes: list, root: str | None) -> dict:
    ...
    for node in nodes:
        ...
        rel = Path(sf_norm)
        # ``Path('/repo/file')`` is not absolute on Windows because it lacks a
        # drive letter, but persisted source_file uses POSIX separators on every
        # host. Treat a leading slash as absolute independently of the host.
