...
@pytest.mark.skipif(os.name == "nt", reason="Windows symlink creation requires Developer Mode or elevated privilege")
...
def test_gitignore_nested_negation_overrides_broader_root_rule(tmp_path):
    ...
    assert any("vendor/sub/important.py" in Path(f).as_posix() for f in code)
...
def test_nested_ignore_overrides_git_info_exclude_and_root(tmp_path):
    ...
    result = detect(tmp_path)
    code = result["files"]["code"]
    assert any("a/b/keep.py" in Path(f).as_posix() for f in code), "nested ! must beat root + info/exclude"
...
def test_path_pattern_single_star_does_not_cross_segment(tmp_path):
    ...
    for pattern in ("/src/*.py", "src/*.py"):
        ...
        assert not any(Path(path).as_posix().endswith("src/main.py") for path in files)
        assert any(Path(path).as_posix().endswith("src/app/main.py") for path in files)
...
def test_detect_surfaces_unreadable_dir_instead_of_silent_skip(tmp_path, capsys):
    ...
    import os
    if os.name == "nt":
        pytest.skip("Windows ACLs do not provide this chmod-based unreadable-directory fixture")
    if os.geteuid() == 0:
