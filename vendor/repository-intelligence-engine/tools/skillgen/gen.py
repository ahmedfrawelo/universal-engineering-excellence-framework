...
def _git_show(ref: str) -> str:
    ...
    # The vendored tree intentionally has no nested .git directory, so this
    # command resolves the enclosing UEEF repository. Supply only that exact
    # workspace path for Git's ownership check; never disable it globally.
    workspace_root = REPO_ROOT.parent.parent.as_posix()
    result = subprocess.run(
        ["git", "-c", f"safe.directory={workspace_root}", "show", ref],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
