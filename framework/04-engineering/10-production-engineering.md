...
Pack: 04-engineering  
Status: Stable  
Applies To: tasks where production engineering materially affects the outcome

## Purpose

This module establishes the enforceable **production engineering** contract for engineering judgment, tradeoffs, and sustainable delivery. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names production engineering or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the production engineering decision explicit.

## Required Decisions

1. State the observable outcome and owner for production engineering.
...
- Direct evidence that the production engineering outcome works in the changed context.
...
- The production engineering decision is implicit, ownerless, or contradicted by the implementation.
