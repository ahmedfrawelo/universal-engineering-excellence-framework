...
Pack: 04-engineering  
Status: Stable  
Applies To: tasks where maintainability materially affects the outcome

## Purpose

This module establishes the enforceable **maintainability** contract for engineering judgment, tradeoffs, and sustainable delivery. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names maintainability or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the maintainability decision explicit.

## Required Decisions

1. State the observable outcome and owner for maintainability.
...
- Direct evidence that the maintainability outcome works in the changed context.
...
- The maintainability decision is implicit, ownerless, or contradicted by the implementation.
