...
Pack: 04-engineering  
Status: Stable  
Applies To: tasks where tradeoff analysis materially affects the outcome

## Purpose

This module establishes the enforceable **tradeoff analysis** contract for engineering judgment, tradeoffs, and sustainable delivery. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names tradeoff analysis or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the tradeoff analysis decision explicit.

## Required Decisions

1. State the observable outcome and owner for tradeoff analysis.
...
- Direct evidence that the tradeoff analysis outcome works in the changed context.
...
- The tradeoff analysis decision is implicit, ownerless, or contradicted by the implementation.
