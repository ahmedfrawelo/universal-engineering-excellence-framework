...
Pack: 04-engineering  
Status: Stable  
Applies To: tasks where problem solving materially affects the outcome

## Purpose

This module establishes the enforceable **problem solving** contract for engineering judgment, tradeoffs, and sustainable delivery. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names problem solving or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the problem solving decision explicit.

## Required Decisions

1. State the observable outcome and owner for problem solving.
...
- Direct evidence that the problem solving outcome works in the changed context.
...
- The problem solving decision is implicit, ownerless, or contradicted by the implementation.
