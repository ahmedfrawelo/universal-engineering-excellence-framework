...
Pack: 04-engineering  
Status: Stable  
Applies To: tasks where reusability materially affects the outcome

## Purpose

This module establishes the enforceable **reusability** contract for engineering judgment, tradeoffs, and sustainable delivery. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names reusability or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the reusability decision explicit.

## Required Decisions

1. State the observable outcome and owner for reusability.
...
- Direct evidence that the reusability outcome works in the changed context.
...
- The reusability decision is implicit, ownerless, or contradicted by the implementation.
