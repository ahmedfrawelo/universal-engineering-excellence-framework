...
Version: 1.0
Pack: 10-frontend/03-ui
Status: Stable
Applies To: tasks where ui materially affects the outcome

## Purpose

This module establishes the enforceable **ui** contract for consistent visual components, states, and design-system reuse. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names ui or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the ui decision explicit.

## Required Decisions

1. State the observable outcome and owner for ui.
2. Use tokens and registered components before creating local visual variants.
3. Record assumptions, rejected alternatives, and the condition that would require revisiting the decision.

## Mandatory Safeguards

- Define loading, empty, error, disabled, focus, and responsive states.
...
- Direct evidence that the ui outcome works in the changed context.
- visual and interaction evidence across supported themes and viewports.
- A focused regression check for the nearest behavior that could be broken.

## Failure Conditions

- The ui decision is implicit, ownerless, or contradicted by the implementation.
