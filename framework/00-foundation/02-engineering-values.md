...
Pack: 00-foundation  
Status: Stable  
Applies To: tasks where engineering values materially affects the outcome

## Purpose

This module establishes the enforceable **engineering values** contract for the framework's governing values and non-negotiable priorities. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names engineering values or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the engineering values decision explicit.

## Required Decisions

1. State the observable outcome and owner for engineering values.
...
- Direct evidence that the engineering values outcome works in the changed context.
...
- The engineering values decision is implicit, ownerless, or contradicted by the implementation.
