...
A task-local `mcp__node_repl__js` failure is a degraded control channel, not evidence that Chrome, the extension, or the dedicated task tab is unavailable. A trusted coordinator may provide evidence for that same target without opening a new browser surface.
...
- `VERIFIED_HANDOFF`: a trusted coordinator used the installed Chrome skill and exact-object `claimTab()` on the dedicated task tab inside the verified existing Chrome window/profile/session and recorded current evidence.
...
If the receiving task changes relevant code after the handoff, it stays active and requests fresh same-target verification. A stale handoff never permits `COMPLETE`; it also never creates a reason to block or ask the user to restart Chrome.

## Gate Rule

For browser or visual work, a current `VERIFIED_HANDOFF` satisfies the same-target verification gate for the dedicated task tab. The task must report the handoff as evidence, not as its own successful local control call. This preserves the user's existing browser, avoids an alternate surface, and prevents a thread-scoped transport fault from stopping engineering work.
