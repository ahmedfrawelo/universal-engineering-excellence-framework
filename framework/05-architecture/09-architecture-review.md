...
Status: Stable  
Applies To: tasks where architecture review materially affects the outcome

## Purpose

This module establishes the enforceable **architecture review** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names architecture review or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the architecture review decision explicit.

## Required Decisions

1. State the observable outcome and owner for architecture review.
...
- Direct evidence that the architecture review outcome works in the changed context.
...
- The architecture review decision is implicit, ownerless, or contradicted by the implementation.
