...
Status: Stable  
Applies To: tasks where separation of concerns materially affects the outcome

## Purpose

This module establishes the enforceable **separation of concerns** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names separation of concerns or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the separation of concerns decision explicit.

## Required Decisions

1. State the observable outcome and owner for separation of concerns.
...
- Direct evidence that the separation of concerns outcome works in the changed context.
...
- The separation of concerns decision is implicit, ownerless, or contradicted by the implementation.
