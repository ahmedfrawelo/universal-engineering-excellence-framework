...
Status: Stable  
Applies To: tasks where feature architecture materially affects the outcome

## Purpose

This module establishes the enforceable **feature architecture** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names feature architecture or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the feature architecture decision explicit.

## Required Decisions

1. State the observable outcome and owner for feature architecture.
...
- Direct evidence that the feature architecture outcome works in the changed context.
...
- The feature architecture decision is implicit, ownerless, or contradicted by the implementation.
