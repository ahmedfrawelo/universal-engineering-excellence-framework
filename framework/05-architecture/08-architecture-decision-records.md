...
Status: Stable  
Applies To: tasks where architecture decision records materially affects the outcome

## Purpose

This module establishes the enforceable **architecture decision records** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names architecture decision records or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the architecture decision records decision explicit.

## Required Decisions

1. State the observable outcome and owner for architecture decision records.
...
- Direct evidence that the architecture decision records outcome works in the changed context.
...
- The architecture decision records decision is implicit, ownerless, or contradicted by the implementation.
