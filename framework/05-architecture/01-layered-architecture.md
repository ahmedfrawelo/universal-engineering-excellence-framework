...
Status: Stable  
Applies To: tasks where layered architecture materially affects the outcome

## Purpose

This module establishes the enforceable **layered architecture** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names layered architecture or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the layered architecture decision explicit.

## Required Decisions

1. State the observable outcome and owner for layered architecture.
...
- Direct evidence that the layered architecture outcome works in the changed context.
...
- The layered architecture decision is implicit, ownerless, or contradicted by the implementation.
