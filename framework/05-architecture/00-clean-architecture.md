...
Status: Stable  
Applies To: tasks where clean architecture materially affects the outcome

## Purpose

This module establishes the enforceable **clean architecture** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names clean architecture or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the clean architecture decision explicit.

## Required Decisions

1. State the observable outcome and owner for clean architecture.
...
- Direct evidence that the clean architecture outcome works in the changed context.
...
- The clean architecture decision is implicit, ownerless, or contradicted by the implementation.
