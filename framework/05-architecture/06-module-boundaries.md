...
Status: Stable  
Applies To: tasks where module boundaries materially affects the outcome

## Purpose

This module establishes the enforceable **module boundaries** contract for system boundaries, dependency direction, and architectural evolution. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names module boundaries or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the module boundaries decision explicit.

## Required Decisions

1. State the observable outcome and owner for module boundaries.
...
- Direct evidence that the module boundaries outcome works in the changed context.
...
- The module boundaries decision is implicit, ownerless, or contradicted by the implementation.
