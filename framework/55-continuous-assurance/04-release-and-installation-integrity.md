...
For GitHub publishing, prefer `scripts/publish-github-release.ps1` after the commit and tag are pushed. The publisher waits for the `Validate UEEF` GitHub Actions workflow on the exact release commit and refuses a failed or unverified run. The script first uses an authenticated `gh` session, then falls back to Git Credential Manager credentials already proven by `git push`; do not start a browser device-login flow merely because `gh` is not authenticated. Never print tokens, passwords, or credential-manager output.
