...
Pass only when the least intrusive suitable control surface is selected, the user's actually opened browser window and target tab are selected through verified visible control or platform-authorized debugging, or a user-requested site is opened in that same browser window and profile, window/profile identity is visibly verified, the visible domain and signed-in state are verified, no automation banner or isolated browser is present, no secrets or browser storage are inspected, the user's window state is preserved, and final state is verified in the same tab.
