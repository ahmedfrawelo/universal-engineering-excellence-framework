...
Pass only when the user's actually opened browser window and target tab are selected through the platform-authorized Chrome connection, window/profile identity is visibly verified, the visible domain and signed-in state are verified, no automation banner or isolated browser is present, no secrets or browser storage are inspected, and final state is verified in the same tab.
