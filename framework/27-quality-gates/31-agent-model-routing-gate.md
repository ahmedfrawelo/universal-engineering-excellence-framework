...
- The selected model and reasoning match the versioned route policy; actual host-agent creation evidence proves which model executed.
- Every materially different work unit was re-routed, its complete technical model identifier and host-provided effort name were visible in an assistant message before dispatch, and every change was visible before the changed execution. Recorder output alone does not satisfy visibility.
...
- Managed route state is bound to a fresh catalog contract, work-unit ID, revision, and digest; model-aware dispatch matches its explicit model/effort.
- Execution receipts derive actual model/effort/result from matching host completion evidence; pre-thread capacity uses an attempt/request ID and fallback receipts are hash-bound to that failure.
...
- T3 fresh-context review decisions are recorded when applicable; T4 fresh-review evidence identifies a distinct reviewer, a `ship` verdict, and an unchanged post-review diff whenever an eligible lane is available.
