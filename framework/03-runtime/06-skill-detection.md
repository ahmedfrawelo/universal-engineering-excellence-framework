...
Pack: 03-runtime  
Status: Stable  
Applies To: tasks where skill detection materially affects the outcome

## Purpose

This module establishes the enforceable **skill detection** contract for the request-to-delivery execution lifecycle. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names skill detection or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the skill detection decision explicit.

## Required Decisions

1. State the observable outcome and owner for skill detection.
...
- Direct evidence that the skill detection outcome works in the changed context.
...
- The skill detection decision is implicit, ownerless, or contradicted by the implementation.
