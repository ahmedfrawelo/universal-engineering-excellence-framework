...
Pack: 03-runtime  
Status: Stable  
Applies To: tasks where request analysis materially affects the outcome

## Purpose

This module establishes the enforceable **request analysis** contract for the request-to-delivery execution lifecycle. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names request analysis or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the request analysis decision explicit.

## Required Decisions

1. State the observable outcome and owner for request analysis.
...
- Direct evidence that the request analysis outcome works in the changed context.
...
- The request analysis decision is implicit, ownerless, or contradicted by the implementation.
