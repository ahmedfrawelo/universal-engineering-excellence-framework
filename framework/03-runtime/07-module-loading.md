...
Pack: 03-runtime  
Status: Stable  
Applies To: tasks where module loading materially affects the outcome

## Purpose

This module establishes the enforceable **module loading** contract for the request-to-delivery execution lifecycle. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names module loading or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the module loading decision explicit.

## Required Decisions

1. State the observable outcome and owner for module loading.
...
- Direct evidence that the module loading outcome works in the changed context.
...
- The module loading decision is implicit, ownerless, or contradicted by the implementation.
