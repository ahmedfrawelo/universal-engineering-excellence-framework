...
Pack: 07-security  
Status: Stable  
Applies To: tasks where api security materially affects the outcome

## Purpose

This module establishes the enforceable **api security** contract for application security, abuse resistance, and data protection. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names api security or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the api security decision explicit.

## Required Decisions

1. State the observable outcome and owner for api security.
...
- Direct evidence that the api security outcome works in the changed context.
...
- The api security decision is implicit, ownerless, or contradicted by the implementation.
