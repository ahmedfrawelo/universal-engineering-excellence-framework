...
Pack: 07-security  
Status: Stable  
Applies To: tasks where secure error handling materially affects the outcome

## Purpose

This module establishes the enforceable **secure error handling** contract for application security, abuse resistance, and data protection. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names secure error handling or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the secure error handling decision explicit.

## Required Decisions

1. State the observable outcome and owner for secure error handling.
...
- Direct evidence that the secure error handling outcome works in the changed context.
...
- The secure error handling decision is implicit, ownerless, or contradicted by the implementation.
