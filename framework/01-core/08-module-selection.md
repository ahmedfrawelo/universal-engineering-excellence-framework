...
Pack: 01-core  
Status: Stable  
Applies To: tasks where module selection materially affects the outcome

## Purpose

This module establishes the enforceable **module selection** contract for UEEF loading, routing, activation, and completion behavior. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names module selection or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the module selection decision explicit.

## Required Decisions

1. State the observable outcome and owner for module selection.
...
- Direct evidence that the module selection outcome works in the changed context.
...
- The module selection decision is implicit, ownerless, or contradicted by the implementation.
