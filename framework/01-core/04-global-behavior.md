...
Pack: 01-core  
Status: Stable  
Applies To: tasks where global behavior materially affects the outcome

## Purpose

This module establishes the enforceable **global behavior** contract for UEEF loading, routing, activation, and completion behavior. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names global behavior or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the global behavior decision explicit.

## Required Decisions

1. State the observable outcome and owner for global behavior.
...
- Direct evidence that the global behavior outcome works in the changed context.
...
- The global behavior decision is implicit, ownerless, or contradicted by the implementation.
