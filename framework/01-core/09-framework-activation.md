...
Pack: 01-core  
Status: Stable  
Applies To: tasks where framework activation materially affects the outcome

## Purpose

This module establishes the enforceable **framework activation** contract for UEEF loading, routing, activation, and completion behavior. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names framework activation or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the framework activation decision explicit.

## Required Decisions

1. State the observable outcome and owner for framework activation.
...
- Direct evidence that the framework activation outcome works in the changed context.
...
- The framework activation decision is implicit, ownerless, or contradicted by the implementation.
