...
8. Generate a schema-version-2 completion-audit artifact from `framework/38-templates/completion-audit-template.json`. Preserve the original goal in `sourceReview.sourceText`, cover all non-whitespace text with contiguous exact review units, classify and link every unit, map every explicit requirement to acceptance criteria and current evidence, and validate it with `scripts/validate-completion-audit.ps1`.
