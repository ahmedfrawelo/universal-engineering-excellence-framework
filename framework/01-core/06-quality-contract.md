...
Pack: 01-core  
Status: Stable  
Applies To: tasks where quality contract materially affects the outcome

## Purpose

This module establishes the enforceable **quality contract** contract for UEEF loading, routing, activation, and completion behavior. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names quality contract or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the quality contract decision explicit.

## Required Decisions

1. State the observable outcome and owner for quality contract.
...
- Direct evidence that the quality contract outcome works in the changed context.
...
- The quality contract decision is implicit, ownerless, or contradicted by the implementation.
