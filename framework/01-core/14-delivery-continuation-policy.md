...
`COMPLETE_ALLOWED = requested_outcome_complete AND no_required_work_remaining AND gates_pass AND verification_recorded AND (host_native_T0_T1 OR completion_audit_passed)`

`FINAL_ALLOWED = user_requested_status_only OR COMPLETE_ALLOWED OR BLOCKED_ALLOWED`

`host_native_T0_T1` requires recorded route metadata before mutation or non-read execution, work performed by the current lead without duplicate App Server dispatch, and a focused direct verification of the bounded outcome. T0/T1 require neither a schema-version-2 completion audit nor the eight-label final block.

For T2+, `completion_audit_passed` requires a schema-version-2 audit whose verbatim source review covers every non-whitespace character of the original goal with contiguous, exact `start`/`end` segments, classifies every segment, links every segment to a requirement, and links every requirement to passing acceptance criteria with current evidence. It also requires an announced `implementation complete -> goal review started` transition while the goal remained `ACTIVE`, an inventory of actual implemented/observed behavior, a checked goal-to-actual-implementation comparison for every requirement, requested-implementation and best-feasible-outcome passes, no untraced implementation, and a changed-surface regression review with no remaining task-caused regression. Unrelated findings are recorded with evidence and an out-of-scope reason but are not repaired. `remainingWork` and `knownProblems` must be empty. Self-declared completion, a prose reread, a green build, or a broad test suite alone cannot satisfy it.
...
For T2+ implementation, when implementation finishes, do not call the goal complete yet. Announce `Implementation complete; goal review started; goal remains ACTIVE` (localized for the user), create the completion checklist, and perform the literal and regression reviews. If review finds a task-caused gap or regression, fix it and repeat the affected review. If review passes, say the goal is complete and stop. T0/T1 instead close after their focused host-native verification. A trailing question such as "anything missing?", "want anything else?", or "is the goal complete?" is forbidden because it reopens a proven bounded goal.
