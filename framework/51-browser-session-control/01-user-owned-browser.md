...
- When the route or browser gate selects Chrome work, the agent does not ask for routine permission to open or control that dedicated task tab. It proceeds through the allowed Chrome binding automatically. Only external missing access, authentication, working-tab takeover, window-state changes, isolated/synthetic testing, or emergency loopback require explicit user action or authorization.
