...
When a user asks to inspect, test, navigate, or operate a website, prefer verified visible Windows control of the existing Chrome window. Use platform-authorized Chrome debugging only when its debugging capabilities are actually required.

## Default Flow

1. Verify the visible active Chrome window and use visible control for ordinary interaction.
2. Request the platform Chrome permission only when debugging-level capabilities are needed.
