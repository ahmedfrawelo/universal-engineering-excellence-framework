...
6. If the task's control channel remains degraded after that one readiness/self-repair pass, request and record a current `VERIFIED_HANDOFF` for the same user-owned tab and current code state. Do not run a visible retry loop, do not report attempt counts, and do not mark `BLOCKED`.
