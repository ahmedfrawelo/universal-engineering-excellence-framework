...
6. If the task's control channel remains degraded after that one readiness/self-repair pass, automatically seek and record a current `VERIFIED_HANDOFF` from an available trusted coordinator for the same user-owned tab and current code state. Do not ask the user to acknowledge the handoff, run a visible retry loop, report attempt counts, or mark `BLOCKED`.
7. If visual verification was requested, keep the task active until the same user-owned tab is claimed and verified locally or through the current handoff. Build, tests, or structural similarity cannot substitute for that gate.
