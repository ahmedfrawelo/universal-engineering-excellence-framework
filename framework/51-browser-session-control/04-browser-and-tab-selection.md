...
- Creating and controlling this dedicated task tab is authorized by the selected browser-required task route. Do not ask the user in chat to approve routine tab creation/control. If the host displays a permission prompt, treat it as platform authorization and continue the readiness flow after it is available.
