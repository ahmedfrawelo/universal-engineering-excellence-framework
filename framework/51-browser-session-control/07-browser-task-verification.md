...
- Verify the visible final state in the dedicated task tab.
- When the request requires browser or visual verification, do not report `COMPLETE` from source inspection, component reuse, build success, tests, or claimed structural equivalence. Keep the task active until required same-target evidence exists for the dedicated task tab. A current `VERIFIED_HANDOFF` is valid when it covers that target and current code state.
