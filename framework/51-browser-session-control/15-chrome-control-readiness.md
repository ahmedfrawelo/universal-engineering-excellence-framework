...
7. If no trusted coordinator channel can supply current same-tab evidence, use verified visible Windows control only on Windows when the Chrome plugin itself is independently unavailable and the same user-owned window can be identified. On macOS/Linux, stop and ask the user to activate or share the existing tab. Do not create a browser, profile, or isolated context.
