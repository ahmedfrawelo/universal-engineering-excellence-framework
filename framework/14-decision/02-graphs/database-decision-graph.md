# Database Decision Graph

## Decision Question

Which option satisfies the required **database** outcome while controlling data invariants, schema compatibility, locking, query plans, migration, and rollback?

## Flow

1. Define the observable outcome, constraints, owner, and evidence threshold.
2. If the repository already has a conforming database mechanism, extend it and verify compatibility; otherwise continue.
3. List the viable options and reject any that cannot make failure, rollback, or ownership explicit.
4. Compare the remaining options on data invariants, schema compatibility, locking, query plans, migration, and rollback.
...
- migration rehearsal, integrity queries, representative plans, and rollback evidence.
