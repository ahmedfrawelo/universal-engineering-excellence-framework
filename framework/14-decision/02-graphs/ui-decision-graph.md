...
Which option satisfies the required **ui** outcome while controlling design-system reuse, semantics, state feedback, keyboard behavior, themes, and responsiveness?

## Flow

1. Define the observable outcome, constraints, owner, and evidence threshold.
2. If the repository already has a conforming ui mechanism, extend it and verify compatibility; otherwise continue.
3. List the viable options and reject any that cannot make failure, rollback, or ownership explicit.
4. Compare the remaining options on design-system reuse, semantics, state feedback, keyboard behavior, themes, and responsiveness.
...
- visual, keyboard, accessibility, and viewport evidence.
