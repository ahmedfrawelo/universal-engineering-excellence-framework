...
Uses modal interruption only when the user must resolve or acknowledge a focused task.

## Required Practice

- Provide an accessible name, initial focus, focus trap, explicit close path, and restoration.
- Disable outside dismissal for destructive confirmation or unsaved critical work when required.
- Prevent stacked modals unless the workflow cannot be represented safely otherwise.
