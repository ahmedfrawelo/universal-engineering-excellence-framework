...
Requires the same trigger to open and close its owned non-modal overlay.

## Required Practice

- Use a single source of truth for trigger expanded state and overlay visibility.
- Handle pointer and keyboard activation through the same command.
- Keep aria-expanded synchronized through animation and teardown.
