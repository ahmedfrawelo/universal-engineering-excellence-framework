...
Provides short supplemental labels without hiding essential content.

## Required Practice

- Show on hover and keyboard focus with a delay that avoids accidental noise.
- Keep tooltips non-interactive; use a popover when actions or rich content are needed.
- Provide accessible names directly for icon-only controls rather than relying on tooltip exposure.
