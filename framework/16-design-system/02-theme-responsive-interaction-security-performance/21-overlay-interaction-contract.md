...
Defines one state machine for trigger, open, dismiss, focus, layering, and cleanup.

## Required Practice

- Give every overlay an owner, type, trigger, dismissal policy, focus policy, and layer.
- Model open and close as idempotent transitions with typed reasons.
- Clean listeners, portals, locks, and focus state on every exit path.
