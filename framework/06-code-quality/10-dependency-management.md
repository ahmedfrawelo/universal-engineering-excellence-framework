...
Status: Stable  
Applies To: tasks where dependency management materially affects the outcome

## Purpose

This module establishes the enforceable **dependency management** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names dependency management or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the dependency management decision explicit.

## Required Decisions

1. State the observable outcome and owner for dependency management.
...
- Direct evidence that the dependency management outcome works in the changed context.
...
- The dependency management decision is implicit, ownerless, or contradicted by the implementation.
