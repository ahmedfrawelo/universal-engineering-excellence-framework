...
Status: Stable  
Applies To: tasks where code smells materially affects the outcome

## Purpose

This module establishes the enforceable **code smells** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names code smells or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the code smells decision explicit.

## Required Decisions

1. State the observable outcome and owner for code smells.
...
- Direct evidence that the code smells outcome works in the changed context.
...
- The code smells decision is implicit, ownerless, or contradicted by the implementation.
