...
Status: Stable  
Applies To: tasks where technical debt materially affects the outcome

## Purpose

This module establishes the enforceable **technical debt** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names technical debt or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the technical debt decision explicit.

## Required Decisions

1. State the observable outcome and owner for technical debt.
...
- Direct evidence that the technical debt outcome works in the changed context.
...
- The technical debt decision is implicit, ownerless, or contradicted by the implementation.
