...
Status: Stable  
Applies To: tasks where refactoring materially affects the outcome

## Purpose

This module establishes the enforceable **refactoring** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names refactoring or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the refactoring decision explicit.

## Required Decisions

1. State the observable outcome and owner for refactoring.
...
- Direct evidence that the refactoring outcome works in the changed context.
...
- The refactoring decision is implicit, ownerless, or contradicted by the implementation.
