...
Status: Stable  
Applies To: tasks where code quality materially affects the outcome

## Purpose

This module establishes the enforceable **code quality** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names code quality or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the code quality decision explicit.

## Required Decisions

1. State the observable outcome and owner for code quality.
...
- Direct evidence that the code quality outcome works in the changed context.
...
- The code quality decision is implicit, ownerless, or contradicted by the implementation.
