...
Status: Stable  
Applies To: tasks where duplication control materially affects the outcome

## Purpose

This module establishes the enforceable **duplication control** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names duplication control or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the duplication control decision explicit.

## Required Decisions

1. State the observable outcome and owner for duplication control.
...
- Direct evidence that the duplication control outcome works in the changed context.
...
- The duplication control decision is implicit, ownerless, or contradicted by the implementation.
