...
Status: Stable  
Applies To: tasks where file size materially affects the outcome

## Purpose

This module establishes the enforceable **file size** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names file size or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the file size decision explicit.

## Required Decisions

1. State the observable outcome and owner for file size.
...
- Direct evidence that the file size outcome works in the changed context.
...
- The file size decision is implicit, ownerless, or contradicted by the implementation.
