...
Status: Stable  
Applies To: tasks where clean code materially affects the outcome

## Purpose

This module establishes the enforceable **clean code** contract for code readability, change safety, and debt control. It exists to turn that concern into explicit decisions, safeguards, and completion evidence.

## Apply When

- The request names clean code or changes behavior governed by it.
- Repository inspection finds an affected boundary, convention, artifact, or risk.
- A reviewer cannot prove the outcome without making the clean code decision explicit.

## Required Decisions

1. State the observable outcome and owner for clean code.
...
- Direct evidence that the clean code outcome works in the changed context.
...
- The clean code decision is implicit, ownerless, or contradicted by the implementation.
