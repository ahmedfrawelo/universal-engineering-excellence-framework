...
Which option satisfies the required **architecture** outcome while controlling boundary ownership, dependency direction, coupling, deployment, and evolution cost?

## Flow

1. Define the observable outcome, constraints, owner, and evidence threshold.
2. If the repository already has a conforming architecture mechanism, extend it and verify compatibility; otherwise continue.
3. List the viable options and reject any that cannot make failure, rollback, or ownership explicit.
4. Compare the remaining options on boundary ownership, dependency direction, coupling, deployment, and evolution cost.
...
- a dependency view, affected scenarios, and an ADR for the selected boundary.
