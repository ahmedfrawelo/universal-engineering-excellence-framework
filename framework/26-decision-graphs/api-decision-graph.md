...
Which option satisfies the required **api** outcome while controlling compatibility, authorization, idempotency, validation, and error contracts?

## Flow

1. Define the observable outcome, constraints, owner, and evidence threshold.
2. If the repository already has a conforming api mechanism, extend it and verify compatibility; otherwise continue.
3. List the viable options and reject any that cannot make failure, rollback, or ownership explicit.
4. Compare the remaining options on compatibility, authorization, idempotency, validation, and error contracts.
...
- contract tests for success, invalid input, forbidden access, retries, and prior clients.
