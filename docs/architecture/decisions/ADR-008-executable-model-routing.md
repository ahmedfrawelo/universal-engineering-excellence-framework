...
Routing is repeated at each materially different work unit. Safe local read-only intake may precede route selection so the tier is evidence-based. Before mutation or non-read execution, the host publishes the route and publishes a replacement line before a changed route. T0/T1 execute in the current host-native lead without a duplicate App Server turn; their recorded route remains attribution and policy evidence. T2-T4 require a matching verified App Server dispatch before substantive execution and completion evidence. Routed turns preserve user language by default and accept a BCP-47 language tag such as Arabic `ar`. Automatic routing ignores the current picker. An explicit current-model constraint is fail-closed and can be removed only by a separate explicit user authorization. The same rule applies to efforts above `high`.
...
- Economical T0/T1 work avoids duplicate cognition and closes with focused host-native verification; T2+ keeps exact model/effort receipt enforcement.
