...
def test_skill_roundtrip_at_real_destination(platform, project, tmp_path, monkeypatch):
    ...
    # Hermes resolves its Windows user install below LOCALAPPDATA rather than
    # Path.home(); keep the round-trip fixture isolated from the host value.
    if platform == "hermes" and os.name == "nt":
