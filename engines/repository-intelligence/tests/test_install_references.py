...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    # Gemini uses ~/.agents/skills on Windows and ~/.gemini/skills on POSIX.
    home_dir = ".agents" if sys.platform == "win32" else ".gemini"
    skill = tmp_path / home_dir / "skills" / "graphify" / "SKILL.md"
    assert skill.exists()
