...
- Added `UEEF-UPSTREAM.json` to make the source revision and containment policy machine-readable.
- Converted the embedded source policy from a complete tracked upstream tree to
  a curated UEEF engine subset. Upstream CI/deploy metadata, Docker/pre-commit
  scaffolding, nested AGENTS guidance, upstream docs/history/meta, translated
...
- Made the embedded skill generator pass only the exact enclosing workspace as
