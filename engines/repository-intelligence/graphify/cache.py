...
def _filesystem_path(path: Path | str) -> str:
    """Return a Windows extended-length path when a cache write needs it."""
    value = str(path)
    # Cache writes append a 64-character digest and random temporary suffix, so
    # promote the directory before the final child reaches MAX_PATH.
    if os.name != "nt" or value.startswith("\\\\?\\") or len(value) < 180:
        return value
...
def file_hash(path: Path, root: Path = Path("."), cache_root: "Path | None" = None) -> str:
    ...
    try:
        ...
        # NTFS can retain identical size and timestamp metadata for rapid
        # same-length writes; never reuse a stat-only hash on Windows.
        if (os.name != "nt" and isinstance(entry, dict)
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast",
    ...
    fd, tmp_path = tempfile.mkstemp(dir=_filesystem_path(target_dir), prefix=f"{h}.", suffix=".tmp")
    try:
        os.write(fd, json.dumps(on_disk).encode())
        os.close(fd)
        try:
            os.replace(tmp_path, _filesystem_path(entry))
        except PermissionError:
            # Windows: os.replace can fail with WinError 5 if the target is
            # briefly locked. Fall back to copy-then-delete.
            import shutil
            shutil.copy2(tmp_path, _filesystem_path(entry))
            os.unlink(tmp_path)
    except Exception:
        try:
            os.close(fd)
