...
def _cap_filename(s: str, limit: int = 128) -> str:
...
def _write_path(path: Path) -> Path:
    """Return a Windows extended-length path when an export exceeds MAX_PATH."""
    value = str(path.resolve())
    if os.name == "nt" and not value.startswith("\\\\?\\") and len(value) >= 240:
        return Path("\\\\?\\" + value)
    return path
...
def to_obsidian(
    ...
    def _owned_write(rel_name: str, content: str) -> bool:
        ...
        _write_path(target).write_text(content, encoding="utf-8")  # nosec
