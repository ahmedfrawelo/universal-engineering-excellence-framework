...
for term in WebSocket Origin SSE Retry-After backpressure no-page-reload; do
  grep -Fq "$term" "$ROOT/framework/16-design-system/02-theme-responsive-interaction-security-performance/51-global-live-refresh.md" || { echo "Global live-refresh contract missing: $term" >&2; exit 1; }
done
for term in 'request waterfalls' 'duplicate vendor chunks' 'reserved layout' 'first user request' 'measured before/after evidence'; do
  grep -Fq "$term" "$ROOT/framework/16-design-system/02-theme-responsive-interaction-security-performance/50-application-lazy-loading.md" || { echo "Application lazy-loading contract missing: $term" >&2; exit 1; }
done
