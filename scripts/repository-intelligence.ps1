param(
  ...
  [ValidateSet('bootstrap', 'build', 'query', 'path', 'explain', 'affected', 'status', 'doctor')]
  [string]$Command,
...
if ($Command -eq 'bootstrap') {
  if (!(Get-Command uv -ErrorAction SilentlyContinue)) { throw 'uv is required to bootstrap repository intelligence. Install uv and retry.' }
  ...
  Write-Output $(if ($needsSync) { 'Repository intelligence dependencies bootstrapped.' } else { 'Repository intelligence dependencies already current.' })
  exit 0
}

if ($needsSync) {
  throw 'Repository intelligence dependencies are missing or stale. Run scripts/repository-intelligence.ps1 -Command bootstrap explicitly, then retry.'
}
