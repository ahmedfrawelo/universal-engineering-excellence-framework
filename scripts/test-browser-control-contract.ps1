...
$required = @{
  ...
  'framework/18-runtime-operations/02-browser-session-control/11-control-surface-selection.md' = @('explicit Chrome-family binding', 'agent.browsers.get("chrome")', 'dedicated task tab', 'getDefault()', 'getForUrl()', 'get("iab")', 'mcp__playwright__*', 'tab.playwright', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'ExpectedTargetId', 'READY_LAST_RESORT', 'sameTargetProven', 'loopback-only', 'A stricter system, host, plugin, or installed-skill rule overrides', 'visible Windows control only on Windows', 'macOS/Linux')
  'framework/18-runtime-operations/02-browser-session-control/09-platform-authorized-chrome-control.md' = @('bootstrap-troubleshooting', 'chrome-troubleshooting', 'Do not invent a `file:///` variant', 'keep the task active', 'file://', '127.0.0.1', 'runtime-sync fix', 'stage/reason/next')
  'framework/18-runtime-operations/02-browser-session-control/12-cross-session-evidence-handoff.md' = @('THREAD_CONTROL_CHANNEL_DEGRADED', 'CHROME_EXTERNALLY_UNAVAILABLE', 'VERIFIED_HANDOFF', 'trusted coordinator', 'dedicated task tab', 'same-target', 'current code state')
  'framework/18-runtime-operations/02-browser-session-control/13-user-facing-recovery-protocol.md' = @('first local bridge failure', 'open Chrome first and then restart Codex', 'stage', 'reason', 'next', 'generic connection/channel failure', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'in-app browser', 'Local File URL Recovery', 'file://', '127.0.0.1', 'runtime sync as a browser repair', 'Explicit Synthetic Playwright Contingency', 'SYNTHETIC_ONLY', 'does not access')
  'framework/18-runtime-operations/02-browser-session-control/14-automatic-tab-ownership-recovery.md' = @('dedicated task tab', 'browser-control session', 'Do not ask the user to Share, Connect, restart Chrome, open another tab, or wait for another task', 'repair-chrome-tab-ownership.ps1', 'user.openTabs()', 'exact returned target object', 'claimTab()', 'one automated recovery', 'without a coordinator or user action')
  'framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md' = @('Chrome readiness flow', 'browser-client.mjs', 'agent.browsers.get("chrome")', 'getDefault()', 'user.openTabs()', 'dedicated task tab', 'working tab by default', 'claimTab()', 'repair-chrome-tab-ownership.ps1', 'stage/reason/next', 'VERIFIED_HANDOFF', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'ExpectedTargetId', 'READY_LAST_RESORT', 'sameTargetProven', 'open Chrome first, then restart Codex', 'current code state', 'chrome.tabs.finalize(...)', 'not enough to prove that Chrome cannot be used', 'Local Navigation Recovery', 'file://', '127.0.0.1', 'Do not run runtime sync', 'Synthetic-Only Contingency', 'SYNTHETIC_ONLY', 'host permits', 'Do not ask the user for routine approval', 'readiness and recovery flow is automatic')
  'framework/18-runtime-operations/02-browser-session-control/16-control-channel-failover.md' = @('Same-Target', 'dedicated task tab', 'Automatic Failover', 'VERIFIED_HANDOFF', 'visible Windows control', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'ExpectedTargetId', 'READY_LAST_RESORT', 'sameTargetProven', 'in-app browser', 'stage and cause category', 'macOS/Linux')
  'framework/18-runtime-operations/02-browser-session-control/07-browser-task-verification.md' = @('do not report `COMPLETE`', 'structural equivalence', 'same-target evidence', 'dedicated task tab', 'chrome.tabs.finalize(...)', 'prevents stale cross-task ownership')
...
$isolatedText = Get-Content -LiteralPath (Join-Path $root 'framework/18-runtime-operations/02-browser-session-control/03-no-isolated-browser-by-default.md') -Raw
if ($isolatedText -match 'Isolated contexts are acceptable') { throw 'Isolated Chrome fallback remains.' }
$checklistText = Get-Content -LiteralPath (Join-Path $root 'framework/12-delivery-quality/06-checklists/32-browser-session-control-checklist.md') -Raw
...
$handoffText = Get-Content -LiteralPath (Join-Path $root 'framework/18-runtime-operations/02-browser-session-control/12-cross-session-evidence-handoff.md') -Raw
...
$browserContractText = Get-ChildItem -LiteralPath (Join-Path $root 'framework/18-runtime-operations/02-browser-session-control') -File -Filter *.md | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw }
...
foreach ($term in @('must ask the user to approve browser control', 'requires user approval before controlling the dedicated task tab', 'wait for browser-control approval before proceeding')) {
  if ($browserContractText -match [regex]::Escape($term)) { throw "Routine browser approval prompt remains: $term" }
}
Assert-TermsInOrder 'framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md' @(
  'browser-client.mjs',
  'claimTab()',
  'repair-chrome-tab-ownership.ps1',
  'VERIFIED_HANDOFF'
)
$readinessText = Get-Content -LiteralPath (Join-Path $root 'framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md') -Raw
foreach ($term in @('platform permission prompt is normal external authorization UI', 'Only report `CHROME_EXTERNALLY_UNAVAILABLE`', 'not enough to prove that Chrome cannot be used', 'cannot become a false `BLOCKED` state')) {
...
foreach ($term in @('localFileUrlRecovery','LOCAL_NAVIGATION_RESTRICTION','controlChannelRemains','syncRepairsBrowser','127.0.0.1','userNoticeRequired')) { if ($preflightText -notmatch [regex]::Escape($term)) { throw "Local file URL recovery term '$term' missing from browser preflight." } }
$browserPolicyText = (@('UEEF-LOADER.md','framework/18-runtime-operations/02-browser-session-control/04-browser-and-tab-selection.md','framework/18-runtime-operations/02-browser-session-control/11-control-surface-selection.md','framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md') | ForEach-Object { Get-Content -LiteralPath (Join-Path $root $_) -Raw }) -join "`n"
