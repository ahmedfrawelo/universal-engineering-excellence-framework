...
$required = @{
  'UEEF-LOADER.md' = @('user.openTabs()', 'claimTab()', 'connector-created Chrome window', 'repair-chrome-tab-ownership.ps1', 'VERIFIED_HANDOFF', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'Host or installed-skill prohibitions still win', 'open Chrome first, then restart Codex', 'non-visual tests can continue', 'keep visual verification explicitly pending', 'minimized, background, or non-foreground')
  ...
  'framework/51-browser-session-control/11-control-surface-selection.md' = @('Chrome plugin extension binding', 'mcp__node_repl__js', 'mcp__playwright__*', 'tab.playwright', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'loopback-only', 'A stricter system, host, plugin, or installed-skill rule overrides', 'visible Windows control only on Windows', 'macOS/Linux')
  ...
  'framework/51-browser-session-control/15-chrome-control-readiness.md' = @('Chrome readiness flow', 'browser-client.mjs', 'not a connector-created browser', 'user.openTabs()', 'claimTab()', 'repair-chrome-tab-ownership.ps1', 'same extension binding', 'VERIFIED_HANDOFF', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'open Chrome first, then restart Codex', 'same tab and current code state', 'continue non-browser work', 'chrome.tabs.finalize(...)', 'not enough to prove that Chrome cannot be used')
  'framework/51-browser-session-control/16-control-channel-failover.md' = @('same user-owned tab', 'Automatic Failover', 'VERIFIED_HANDOFF', 'visible Windows control', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'macOS/Linux', 'never creates')
  ...
  'framework/03-runtime/00-runtime-sequence.md' = @('Chrome readiness flow completed:', 'Exact user.openTabs() object claimed:', 'Automatic ownership repair run when needed:', 'Emergency fallback authorization:', 'READY_LAST_RESORT', 'LOOPBACK_ONLY', 'Browser storage inspected: NO', 'Banner classification:', 'PARTIAL_VISUAL_GATE')
  ...
  'scripts/sync-runtime.ps1' = @('user.openTabs()', 'claimTab()', 'Chrome readiness flow', 'THREAD_CONTROL_CHANNEL_DEGRADED', 'VERIFIED_HANDOFF', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'open Chrome first, then restart Codex', 'Do not expose retry counts', 'repair-chrome-tab-ownership.ps1')
...
foreach ($term in @("status = 'REQUIRED'", "enforcement = 'HARD_FAIL_BEFORE_BROWSER_TOOL'", 'requiredBeforeTool', 'allowedPath', 'forbiddenSurfaces', 'emergencyFallback', 'AUTHORIZED_LOOPBACK_LAST_RESORT', 'READY_LAST_RESORT', 'hostRulesWin', 'Do not select or call a browser tool')) {
