...
$required = @{
  ...
  'framework/51-browser-session-control/04-browser-and-tab-selection.md' = @('user.openTabs()', 'dedicated task tab', 'same existing window/profile/session', 'explicitly asks to use that tab', 'claimTab()', 'not permission to take it over', 'Do not ask the user in chat to approve routine tab creation/control')
  ...
  'framework/51-browser-session-control/09-platform-authorized-chrome-control.md' = @('bootstrap-troubleshooting', 'chrome-troubleshooting', 'Do not invent a `file:///` variant', 'keep the task active', 'file://', '127.0.0.1', 'runtime-sync fix', 'stage/reason/next')
