...
print_section() {
  ...
  results="$(
    for name in "$@"; do
      find "$ROOT" \( -type d -name .git \) -prune -o \( -type d \( -name vendor -o -name node_modules -o -name dist -o -name build -o -name out -o -name coverage -o -name .next -o -name .angular -o -name bin -o -name obj \) \) -prune -o -name "$name" -type "$type" -print 2>/dev/null
...
print_generated_section() {
  ...
  results="$(
    find "$ROOT" \( -type d -name .git \) -prune -o -type d \( -name vendor -o -name dist -o -name build -o -name out -o -name coverage -o -name .next -o -name .angular -o -name node_modules -o -name bin -o -name obj -o -name logs -o -name screenshots -o -name artifacts \) -print -prune 2>/dev/null |
      sed "s#^$ROOT/##" | sort -u | head -n "$MAX_ITEMS" || true
  )"
  if [ -n "$results" ]; then printf '%s\n' "$results" | sed 's/^/- /'; else echo "- none detected"; fi
}
