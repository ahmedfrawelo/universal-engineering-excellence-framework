...
print_section() {
  ...
  results="$(
    for name in "$@"; do
      find "$ROOT" \( -type d -name .git \) -prune -o \( -type d \( -name vendor -o -name engines -o -name graphify-out -o -name node_modules -o -name dist -o -name build -o -name out -o -name coverage -o -name .next -o -name .angular -o -name bin -o -name obj \) \) -prune -o -name "$name" -type "$type" -print 2>/dev/null
...
print_test_section() {
  ...
  results="$(
    find "$ROOT" \( -type d -name .git \) -prune -o \( -type d \( -name vendor -o -name engines -o -name graphify-out -o -name node_modules -o -name dist -o -name build -o -name out -o -name coverage -o -name .next -o -name .angular -o -name bin -o -name obj \) \) -prune -o \( \( -type d \( -name test -o -name tests -o -name e2e -o -name spec -o -name specs -o -name __tests__ -o -name playwright -o -name cypress \) \) -o \( -type f \( -name 'test-*' -o -name '*.test.*' -o -name '*_test.*' -o -name '*Tests.*' \) \) \) -print 2>/dev/null |
...
print_generated_section() {
  ...
  results="$(
    find "$ROOT" \( -type d -name .git \) -prune -o -type d \( -name vendor -o -name engines -o -name graphify-out -o -name dist -o -name build -o -name out -o -name coverage -o -name .next -o -name .angular -o -name node_modules -o -name bin -o -name obj -o -name logs -o -name screenshots -o -name artifacts \) -print -prune 2>/dev/null |
      sed "s#^$ROOT/##" | sort -u | head -n "$MAX_ITEMS" || true
  )"
  if [ -n "$results" ]; then printf '%s\n' "$results" | sed 's/^/- /'; else echo "- none detected"; fi
}
...
if [ -f "$ROOT/.ueef/repository-graph/state.json" ]; then
  ...
  echo "Repository intelligence: NOT_BUILT - repository-scoped UEEF work should build it automatically before substantial work"
fi
