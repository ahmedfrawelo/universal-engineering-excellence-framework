...
import { hookRoot, peekWorkUnitInvocation, readTurnState, sha256Text, updateTurnState } from './ueef-hook-common.mjs';
...
if (routeOutput) {
  const resolvedOutput = path.resolve(routeOutput);
  fs.mkdirSync(path.dirname(resolvedOutput), { recursive: true });
  fs.writeFileSync(resolvedOutput, `${JSON.stringify({
    ...modelRoute,
    workUnitId,
    routeDigest,
    routeRevision,
...
process.stdout.write(`${JSON.stringify({
  ...
  routeDigest,
  routeOutput: routeOutput ? path.resolve(routeOutput) : null
})}\n`);
