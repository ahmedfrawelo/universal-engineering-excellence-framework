...
if (!$narrowRepository.repositoryIntelligence.selected -or !$narrowRepository.repositoryIntelligence.enforced -or $narrowRepository.repositoryIntelligence.action -notin @('USED_FRESH','REFRESHED_AND_USED','BUILT_AND_USED') -or $narrowRepository.repositoryIntelligence.queryReport.status -notin @('PASS','NO_MATCH')) { throw 'Repository-scoped narrow task did not automatically check and use repository intelligence.' }
