...
$record=[ordered]@{schemaVersion=1;id=[guid]::NewGuid().ToString('N');kind=$Kind;title=$Title;detail=$Detail;createdAt=(Get-Date).ToUniversalTime().ToString('o');expiresAt=(Get-Date).ToUniversalTime().AddDays($RetentionDays).ToString('o');source='explicit-user-or-agent-record';automaticPolicyChange=$false}
