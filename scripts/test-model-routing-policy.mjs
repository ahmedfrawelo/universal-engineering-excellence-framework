...
assert.equal(resolve('--tier', 'T0').tokenEconomy.specRequired, false);
assert.equal(resolve('--tier', 'T2').tokenEconomy.specRequired, true);
assert.equal(resolve('--tier', 'T2').tokenEconomy.budgetMode, 'bounded');
assert.equal(resolve('--tier', 'T2').tokenEconomy.delegationPolicy, 'sidecar');
assert.equal(resolve('--tier', 'T3').tokenEconomy.maxWorkerCount, 3);
assert.equal(resolve('--tier', 'T4').tokenEconomy.workerOutputCap.maxWords, 250);
...
const parsedProductionPolicy = JSON.parse(productionPolicy);
assert.equal(parsedProductionPolicy.models, undefined);
assert.deepEqual(parsedProductionPolicy.tokenEconomy.specFirstForTiers, ['T2', 'T3', 'T4']);
assert.equal(parsedProductionPolicy.tokenEconomy.defaultBudgetModeByTier.T0, 'minimal');
assert.equal(parsedProductionPolicy.tokenEconomy.defaultBudgetModeByTier.T2, 'bounded');
assert.equal(parsedProductionPolicy.tokenEconomy.defaultBudgetModeByTier.T4, 'expanded');
assert.equal(parsedProductionPolicy.tokenEconomy.maxWorkerCountByTier.T1, 0);
assert.equal(parsedProductionPolicy.tokenEconomy.maxWorkerCountByTier.T2, 1);
assert.equal(parsedProductionPolicy.tokenEconomy.defaultDelegationPolicyByTier.T2, 'sidecar');
assert.equal(parsedProductionPolicy.tokenEconomy.requireAutomaticExecutionSpec, true);
assert.equal(parsedProductionPolicy.tokenEconomy.workerOutputCap.longEvidenceStoredInArtifacts, true);
assert.equal(parsedProductionPolicy.tokenEconomy.forbiddenSavings.includes('omit-required-acceptance-evidence'), true);
