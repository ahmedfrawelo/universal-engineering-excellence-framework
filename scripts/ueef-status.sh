...
if [ "$managed_runtime" = "1" ]; then
  ...
  active_state_pass=0
  if [ -f "$state_path" ] && node "$REPOSITORY_PATH/scripts/active-state.mjs" validate "$state_path" "$version" "$(basename "$REPOSITORY_PATH")" >/dev/null; then active_state_pass=1; fi
