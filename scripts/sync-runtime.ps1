...
Write-Utf8File $stagingLoader @(
  ...
  "- Resolve config/model-routing-policy.json for every task and materially different work unit, discover concrete model names and supported efforts from the signed-in host, show the route before dispatch, and show any changed route before changed execution. Do not retain the primary conversation's selected model unless the user explicitly requires it; model-constraint override and above-high reasoning each require separate explicit authorization.",
  ...
  "- T3 records whether fresh-context review is needed; T4 requires passing validate-fresh-review-evidence.ps1 before completion whenever an eligible review lane is available. A post-review change invalidates the verdict.",
...
$managedAgentsLines = @(
  ...
  "Before non-trivial work, read the loader, verify status, select relevant modules/tools, and record Intent, Tier, Spawn reason, and Browser reason as route rationale.",
  ...
  "T4 fresh review: validate the distinct reviewer, observed runtime/sandbox context, ship verdict, and unchanged post-review diff with scripts/validate-fresh-review-evidence.ps1 before completion.",
  "Managed hooks enforce routing, protected paths, evidence, completion, progress, and goal closure.",
  ...
  "Long-goal updates need phase, step percent, overall percent, new evidence, current action, and next gate; 100 percent means complete.",
  "",
  "Browser hard stop: Prefer the installed Chrome control plugin for explicit tasks. Never launch Playwright, chrome-devtools, IDE Simple Browser, a second profile, or a new context. After all same-tab paths fail, user-approved loopback CDP may attach to existing Chrome only; otherwise stop and ask.",
  "",
