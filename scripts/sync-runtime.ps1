...
$managedAgentsLines = @(
  ...
  "- If the user-owned tab cannot be proven, STOP without opening another browser. Visible Windows control is Windows-only when the Chrome plugin is independently unavailable; on macOS/Linux ask for the existing tab.",
  "",
...
$managedAgentsLines = @(
  ...
  "",
  "Precedence: Scope wins; stop when done; T1 defaults to single-agent; medium is the economical default, not a hard ceiling; browser control is explicit-task only.",
  ...
  "",
  "Browser hard stop: only the existing user-owned Chrome tab through mcp__node_repl__js -> user.openTabs() -> claimTab() -> tab.playwright. Never create a browser/profile/context. Windows control is Windows-only; on macOS/Linux stop and ask for the existing tab.",
  "",
