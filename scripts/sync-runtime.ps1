...
Write-Utf8File $stagingLoader @(
  ...
  "- Browser work means the user's existing Chrome only - never a second browser, profile, context, Cursor/IDE Simple Browser, in-app browser, or connector-created window. Never use directly exposed mcp__playwright__*, mcp__chrome_devtools__*, browser_* tools, browser.newContext, or browser.launch as substitutes. Playwright is allowed only through a claimed tab.playwright API when the Claude bridge is in use. An isolated/local browser test requires an explicit separate user request and never substitutes for a user-owned Chrome task. Visible Windows control is Windows-only when the Chrome plugin is independently unavailable; on macOS/Linux STOP and request the user to activate or share the existing tab.",
