...
Write-Utf8File $loader @(
  ...
  "- For Chrome, use the Chrome plugin extension binding, enumerate user.openTabs(), and claimTab() the verified user-owned tab. Use visible Windows control only if the plugin is unavailable. Never open another browser surface as recovery.",
...
Write-Utf8File $agents @(
  ...
  "- For Chrome, use the Chrome plugin extension binding, enumerate user.openTabs(), and claimTab() the verified user-owned tab. Extension attachment to an existing tab is allowed and must not be treated as a connector-created window.",
