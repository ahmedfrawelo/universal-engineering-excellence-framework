...
Write-Utf8File $stagingLoader @(
  ...
  "- HARD FAIL BEFORE ANY BROWSER TOOL: only when the task explicitly needs the browser. For a task tagged browser, run scripts/get-ueef-task-preflight.ps1 -Task <task> -TaskTag browser and resolve browserGate before selecting or calling any browser tool. Prefer the platform Chrome control plugin against the user's existing tabs/profile (Codex Chrome plugin, or Claude mcp__node_repl__js -> user.openTabs() -> claimTab() -> tab.playwright when that bridge is present). If the gate is absent, unresolved, or Chrome control is unavailable, do not select a browser tool and do not invent a fallback browser.",
  ...
  "- Browser work means the user's existing Chrome only - never a second browser, profile, context, Cursor/IDE Simple Browser, in-app browser, or connector-created window. Direct Playwright, browser_*, browser.newContext, and browser.launch remain forbidden. An isolated/local test requires an explicit separate user request and never substitutes for this task. A Chrome DevTools/CDP adapter is allowed only as AUTHORIZED_LOOPBACK_LAST_RESORT after every configured same-tab stage has recorded failure evidence, explicit user authorization, and scripts/get-remote-debugging-readiness.ps1 -AuthorizedLastResort -PriorStageFailure <recorded-stages> returns READY_LAST_RESORT; attach to the same existing target on loopback only, never launch or inspect cookies/storage/profile data, and obey stricter host or installed-skill rules. Visible control is Windows-only; on macOS/Linux skip it and use only an eligible authorized loopback stage or stop.",
...
$managedAgentsLines = @(
  ...
  "Long goals: updates need both a conservative percent and an explicit phase, evidence, current action, and next gate; 100 percent means complete.",
  "",
