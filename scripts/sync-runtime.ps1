...
Write-Utf8File $loader @(
  ...
  "- For Chrome, use the Chrome plugin extension binding, enumerate user.openTabs(), and claimTab() the verified user-owned visible tab. Use visible Windows control only if the plugin is unavailable. Never open another browser surface as recovery.",
  "",
...
Write-Utf8File $agents @(
  ...
  "- For Chrome, use the Chrome plugin extension binding, enumerate user.openTabs(), and claimTab() the verified user-owned visible tab. Extension attachment to an existing tab is allowed and must not be treated as a connector-created window.",
  ...
  "- If the visible tab cannot be proven, stop without opening another browser. Use visible Windows control only when the Chrome plugin is unavailable and identity is verified.",
  "",
