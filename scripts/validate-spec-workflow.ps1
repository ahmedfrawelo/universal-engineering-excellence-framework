...
if ($Mode -eq 'Ready' -and $convergence) {
  $requirements = [regex]::Matches((Get-Content -LiteralPath (Join-Path $workflowPath 'spec.md') -Raw), '(?m)^\s*(REQ-[0-9]+):') | ForEach-Object { $_.Groups[1].Value }
  $acceptance = [regex]::Matches((Get-Content -LiteralPath (Join-Path $workflowPath 'spec.md') -Raw), '(?m)^\s*(AC-[0-9]+):') | ForEach-Object { $_.Groups[1].Value }
