...
POLICY="$ROOT/framework/18-runtime-operations/02-browser-session-control/11-control-surface-selection.md"
LOADER="$ROOT/framework/01-core/01-master-loader.md"
GATE="$ROOT/framework/12-delivery-quality/04-quality-gates/23-browser-session-control-gate.md"
RECOVERY="$ROOT/framework/18-runtime-operations/02-browser-session-control/09-platform-authorized-chrome-control.md"
...
for term in 'current window size' 'monitor placement' 'zoom' 'tab order' 'active tab' 'Do not call resize' 'Record the initial and final window state'; do
  grep -Fq "$term" "$ROOT/framework/18-runtime-operations/02-browser-session-control/10-window-state-preservation.md" || { echo "Missing window-geometry preservation term: $term" >&2; exit 1; }
done

! grep -Fq 'Isolated contexts are acceptable' "$ROOT/framework/18-runtime-operations/02-browser-session-control/03-no-isolated-browser-by-default.md" || { echo 'Isolated Chrome fallback remains' >&2; exit 1; }
! grep -Fq 'Explicit consent recorded if an isolated fallback was necessary' "$ROOT/framework/12-delivery-quality/06-checklists/32-browser-session-control-checklist.md" || { echo 'Consent-based isolated fallback remains' >&2; exit 1; }
...
for term in 'Chrome readiness flow' 'browser-client.mjs' 'user.openTabs()' 'claimTab()' 'THREAD_CONTROL_CHANNEL_DEGRADED' 'CHROME_EXTERNALLY_UNAVAILABLE' 'VERIFIED_HANDOFF' 'chrome.tabs.finalize(...)' 'not enough to prove that Chrome cannot be used' 'cannot become a false `BLOCKED` state'; do
  grep -Fq "$term" "$ROOT/framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md" || { echo "Missing Chrome readiness term: $term" >&2; exit 1; }
done
...
grep -Fq 'do not report `COMPLETE`' "$ROOT/framework/18-runtime-operations/02-browser-session-control/07-browser-task-verification.md" || { echo 'Missing required-visual completion guard' >&2; exit 1; }
grep -Fq 'chrome.tabs.finalize(...)' "$ROOT/framework/18-runtime-operations/02-browser-session-control/07-browser-task-verification.md" || { echo 'Missing stale-tab ownership cleanup rule' >&2; exit 1; }
for term in 'THREAD_CONTROL_CHANNEL_DEGRADED' 'CHROME_EXTERNALLY_UNAVAILABLE' 'VERIFIED_HANDOFF' 'trusted coordinator' 'dedicated task tab' 'same-target' 'current code state'; do
  grep -Fq "$term" "$ROOT/framework/18-runtime-operations/02-browser-session-control/12-cross-session-evidence-handoff.md" || { echo "Missing evidence-handoff term: $term" >&2; exit 1; }
done
for term in 'first local bridge failure' 'stage' 'reason' 'next' 'generic connection/channel failure' 'AUTHORIZED_LOOPBACK_LAST_RESORT' 'in-app browser'; do
  grep -Fq "$term" "$ROOT/framework/18-runtime-operations/02-browser-session-control/13-user-facing-recovery-protocol.md" || { echo "Missing user-facing recovery term: $term" >&2; exit 1; }
done
...
for term in 'Same-Target' 'dedicated task tab' 'Automatic Failover' 'VERIFIED_HANDOFF' 'visible Windows control' 'in-app browser' 'stage and cause category' 'No user acknowledgement'; do
  grep -Fq "$term" "$ROOT/framework/18-runtime-operations/02-browser-session-control/16-control-channel-failover.md" || { echo "Missing control-channel failover term: $term" >&2; exit 1; }
done

if grep -Eiq 'wait for the user to say|ask the user to say|say [`'"'"']?ØªÙ…' "$ROOT"/framework/18-runtime-operations/02-browser-session-control/*.md; then
  echo 'Manual user-confirmation browser handoff remains' >&2
  exit 1
fi

readiness="$ROOT/framework/18-runtime-operations/02-browser-session-control/15-chrome-control-readiness.md"
previous=0
