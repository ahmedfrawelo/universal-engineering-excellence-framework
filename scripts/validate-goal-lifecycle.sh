#!/usr/bin/env sh
set -eu
status=ACTIVE; terminal=false; status_only=false; external=false; no_work=false; state_change=false; outcome=false; remaining=false; gates=false; verified=false
while [ "$#" -gt 0 ]; do
  case "$1" in
...
blocked=false; complete=false; allowed=false
[ "$status" = BLOCKED ] && [ "$external" = true ] && [ "$no_work" = true ] && [ "$state_change" = true ] && blocked=true
[ "$status" = COMPLETE ] && [ "$outcome" = true ] && [ "$remaining" = false ] && [ "$gates" = true ] && [ "$verified" = true ] && complete=true
